...
PrimitiveComponent PrimitiveFactory::createRect(glm::vec4 color)
{
    ...
    std::shared_ptr shader = std::make_shared<Shader>("res/shaders/default.vert", "res/shaders/default.frag");
    shader->bind();
...
PrimitiveComponent PrimitiveFactory::createPolygon(glm::vec4 color, unsigned int vertexCount)
{
    ...
    {
        return glm::vec2(point.x * cos(rot) - point.y * sin(rot), point.x * sin(rot) + point.y * cos(rot));
    };
