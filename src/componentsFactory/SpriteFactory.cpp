...
SpriteComponent SpriteFactory::createSprite(std::shared_ptr<Texture> texture, bool keepAspect) {
    std::shared_ptr shader =
        std::make_shared<Shader>("res/shaders/texture.vert", "res/shaders/texture.frag");
    return createSprite(texture, shader, keepAspect);
}

SpriteComponent SpriteFactory::createSprite(std::shared_ptr<Texture> texture,
                                            std::shared_ptr<Shader> shader, bool keepAspect) {
    std::array<float, 20> vertices;
    float aspect =
        static_cast<float>(texture->getWidth()) / static_cast<float>(texture->getHeight());
    if (keepAspect)
        vertices = {
            -0.5f * aspect, 0.5f,  0.0f, 0.0f, 1.0f, 0.5f * aspect, 0.5f,  0.0f, 1.0f, 1.0f,
            -0.5f * aspect, -0.5f, 0.0f, 0.0f, 0.0f, 0.5f * aspect, -0.5f, 0.0f, 1.0f, 0.0f};
    else
        vertices = {-0.5f, 0.5f,  0.0f, 0.0f, 1.0f, 0.5f, 0.5f,  0.0f, 1.0f, 1.0f,
                    -0.5f, -0.5f, 0.0,  0.0f, 0.0f, 0.5f, -0.5f, 0.0f, 1.0f, 0.0f};
