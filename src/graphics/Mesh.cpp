...
void Submesh::draw(std::shared_ptr<Shader> shader) const {
    ...
    vao->bind();
    vao->bindIndex();
    glDrawElements(GL_TRIANGLES, static_cast<unsigned int>(vao->getIndexBuffer()->getCount()),
                   GL_UNSIGNED_INT, 0);
    vao->unbindIndex();
    vao->unbind();
}
...
int Mesh::load(const std::string& filepath) {
    ...
    for (const auto& shape : shapes) {
        ...
        auto ebo = std::make_shared<IndexBuffer>(indices.data(),
        ...
        if (materialId >= 0) {
            ...
            if (!material.diffuse_texname.empty()) {
                submesh.texture =
                    std::make_pair(std::string("textureDiffuse"),
